...
export default function AnnualSpecialExpensesModal({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      for (let i = 1; i <= 30; i++) { // Increased to 30 years
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-[#0F172A]/40 backdrop-blur-sm p-4">
      <div className="bg-[#F8FAFC] rounded-[8px] shadow-2xl w-full max-w-[600px] flex flex-col border border-[#CBD5E1] overflow-hidden max-h-[95vh]">
        ...
        <div className="p-4 overflow-x-auto overflow-y-auto">
          {/* THE FIX: Changed min-w-[500px] to min-w-[450px] so it fits better on phones while still allowing horizontal swipe */}
          ...
          {/* Inflation Block */}
          {/* THE FIX: Removed the strict min-w-[500px] and added w-full so it flexes properly on mobile */}
          <div className="border border-[#CBD5E1] rounded-[6px] p-4 pt-5 bg-white relative w-full">
             <span className="absolute -top-2.5 left-3 bg-white px-1 text-[12px] font-bold text-[#64748B]">Projecting Normal Expenses</span>
             
             {/* THE FIX: Added flex-wrap so the inputs drop down to a new line cleanly on narrow screens */}
             <div className="flex flex-wrap items-center gap-3">
               ...
               <div className="w-[60px] relative">
                 <input 
                   type="text" 
                   value={inflationStartYear} 
                   onChange={(e) => setInflationStartYear(e.target.value)} 
                   className="w-full border border-[#CBD5E1] rounded-[4px] pl-2 pr-5 py-1 text-[13px] focus:outline-none focus:border-[#0052CC]" 
                 />
               ...
               <div className="w-[70px] relative">
                 <input 
                   ...
                   value={inflationRate} 
                   disabled 
                   className="w-full border border-[#CBD5E1] bg-[#F1F5F9] rounded-[4px] pl-2 pr-5 py-1 text-[13px] text-right text-[#64748B]" 
                 />
        ...
        <div className="px-4 py-3 bg-[#F1F5F9] border-t border-[#E2E8F0] flex flex-col-reverse sm:flex-row justify-between items-center gap-4 rounded-b-[8px] shrink-0">
          <div className="flex items-center gap-1 w-full sm:w-auto justify-center">
             <button onClick={handleStart} className="w-8 h-8 flex items-center justify-center border border-[#CBD5E1] bg-white rounded-[4px] text-[13px] text-[#64748B] font-bold hover:bg-[#E2E8F0] shadow-sm transition-colors" title="Start">&lt;&lt;</button>
             <button onClick={handlePrev} className="w-8 h-8 flex items-center justify-center border border-[#CBD5E1] bg-white rounded-[4px] text-[13px] text-[#64748B] font-bold hover:bg-[#E2E8F0] shadow-sm transition-colors" title="Previous">&lt;</button>
             <button onClick={handleNext} className="w-8 h-8 flex items-center justify-center border border-[#CBD5E1] bg-white rounded-[4px] text-[13px] text-[#64748B] font-bold hover:bg-[#E2E8F0] shadow-sm transition-colors" title="Next">&gt;</button>
             <button onClick={() => setStartIndex(25)} className="w-8 h-8 flex items-center justify-center border border-[#CBD5E1] bg-white rounded-[4px] text-[13px] text-[#64748B] font-bold hover:bg-[#E2E8F0] shadow-sm transition-colors" title="End">&gt;&gt;</button>
          </div>
