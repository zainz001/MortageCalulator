...
export default function AnnualInterestRatesModal({
    ...
    return (
      <div className={`mb-4 ${isDisabled ? "opacity-50 pointer-events-none" : ""}`}>
        ...
        <div className="flex gap-4 items-center mb-1">
          <div className="w-[70px] shrink-0 text-right text-[12px] text-[#4A5568]">Rates</div>
          {[0, 1, 2, 3, 4].map(i => (
        ...
        {/* Interest Row */}
        <div className="flex gap-4 items-center mb-1">
          <div className="w-[70px] shrink-0 text-right text-[12px] text-[#4A5568]">Interest</div>
          {[0, 1, 2, 3, 4].map(i => {
            const { interest } = calcYear(loanData, rates[i]);
            return <div key={i} className="flex-1 min-w-[70px] text-right text-[12px] text-[#1E293B]">{isDisabled ? "" : formatVal(interest)}</div>
          })}
        </div>

        {/* Principal Row */}
        <div className="flex gap-4 items-center">
          <div className="w-[70px] shrink-0 text-right text-[12px] text-[#4A5568]">Principal</div>
          {[0, 1, 2, 3, 4].map(i => {
            const { interest, payment } = calcYear(loanData, rates[i]);
            const principal = payment > 0 ? payment - interest : 0;
            return <div key={i} className="flex-1 min-w-[70px] text-right text-[12px] text-[#1E293B]">{isDisabled ? "" : (principal > 0 ? formatVal(principal) : "")}</div>
          })}
        </div>
      </div>
    );
  ...
  return ReactDOM.createPortal(
    /* Wrapper with padding to keep modal off screen glass on mobile */
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-[#0F172A]/40 backdrop-blur-sm p-4">
      {/* Modal Box: w-full max-w-[600px] and max-h for small height screens */}
      <div className="bg-[#F8FAFC] rounded-[8px] shadow-2xl w-full max-w-[600px] flex flex-col border border-[#CBD5E1] overflow-hidden max-h-[95vh]">
        ...
        {/* Main Body: Added vertical scroll */}
        <div className="p-4 pt-2 overflow-y-auto">
          
          {/* THE FIX: Horizontal scroll container for the spreadsheet data */}
          <div className="overflow-x-auto pb-4">
            <div className="min-w-[500px]">
              
              {/* Header Row */}
              <div className="flex gap-4 mb-2 border-b border-[#E2E8F0] pb-2">
                <div className="w-[70px] shrink-0 text-left text-[12px] text-[#4A5568] font-bold">Year</div>
                {[1, 2, 3, 4, 5].map(yr => (
                  <div key={yr} className="flex-1 min-w-[70px] text-center">
              ...
              <div className="mt-4 pt-3 border-t border-[#CBD5E1]">
                <div className="flex gap-4 items-center mb-1">
                  <div className="w-[70px] shrink-0 text-right text-[12px] font-bold text-[#1E293B]">Totals</div>
                  {[0, 1, 2, 3, 4].map(i => (
                    <div key={i} className="flex-1 min-w-[70px] text-center text-[12px] font-bold text-[#1E293B]">
                ...
                <div className="flex gap-4 items-center mb-1">
                  ...
                  {[0, 1, 2, 3, 4].map(i => {
                    ...
                    return <div key={i} className="flex-1 min-w-[70px] text-center text-[12px] text-[#1E293B]">{formatVal(intA + intB)}</div>
                  })}
                </div>

                <div className="flex gap-4 items-center mb-2">
                  <div className="w-[70px] shrink-0 text-right text-[12px] text-[#4A5568]">Payment</div>
                  {[0, 1, 2, 3, 4].map(i => {
                    const pmtA = calcYear(loanA, ratesA[i]).payment;
                    const pmtB = splitRate ? calcYear(loanB, ratesB[i]).payment : 0;
                    return <div key={i} className="flex-1 min-w-[70px] text-center text-[12px] text-[#1E293B]">{formatVal(pmtA + pmtB)}</div>
                  })}
                </div>

                <div className="flex gap-4 items-center">
                  <div className="w-[70px] shrink-0 text-right text-[12px] text-[#4A5568] leading-tight">Average rate</div>
                  {[0, 1, 2, 3, 4].map(i => {
        ...
        {/* Footer: Added flex-col-reverse sm:flex-row to stack buttons on mobile */}
        <div className="px-4 py-3 bg-[#F1F5F9] border-t border-[#E2E8F0] flex flex-col-reverse sm:flex-row justify-between items-center gap-4 sm:gap-2 rounded-b-[8px] shrink-0">
          <div className="flex gap-1 w-full sm:w-auto justify-center">
             <button className="px-3 py-1 border border-[#CBD5E1] bg-white rounded-[3px] text-[12px] text-[#1E293B] shadow-sm hover:bg-slate-50 transition-colors">&lt;&lt;</button>
             <button className="px-3 py-1 border border-[#CBD5E1] bg-white rounded-[3px] text-[12px] text-[#1E293B] shadow-sm hover:bg-slate-50 transition-colors">&lt;</button>
             <button className="px-3 py-1 border border-[#CBD5E1] bg-white rounded-[3px] text-[12px] text-[#1E293B] shadow-sm hover:bg-slate-50 transition-colors">&gt;</button>
             <button className="px-3 py-1 border border-[#CBD5E1] bg-white rounded-[3px] text-[12px] text-[#1E293B] shadow-sm hover:bg-slate-50 transition-colors">&gt;&gt;</button>
          </div>
          <div className="flex gap-2 w-full sm:w-auto">
            ...
            <button onClick={onClose} className="flex-1 sm:flex-none px-5 py-1.5 border border-[#CBD5E1] bg-white rounded-[4px] text-[13px] text-[#1E293B] hover:bg-slate-50 transition-colors shadow-sm">Cancel</button>
          </div>
        </div>
